...
(function() {
  var FLOW_RATE, Glass, HEIGHT, MM_HEIGHT, MM_PER_PIXEL, MM_RULER, PIXEL_PER_MM, RULER_LENGTH, RULER_WIDTH, WGlass, WGlassGrafter, WHorizontalRuler, WIDTH, WVerticalRuler, canvas, cbutton, cocktail_glass, cocktail_json, cocktail_representation, conf, csbutton, ebutton, erlenmeyer_glass, erlenmeyer_json, erlenmeyer_representation, esbutton, export_glass, g, graphs, hruler, make_graph, selected, vruler;
  ...
  make_graph = function(conf) {
    var CONTAINER_DIMENSIONS, GRAPH_DIMENSIONS, MARGINS, X_QUANTITY, Y_QUANTITY, Z_QUANTITY, all_graphs, draw_chart, draw_graph, draw_tangents, graph, id, svg_graph, tangent, toggle_chart, toggle_graph, toggle_tangents, x_axis, x_extent, x_scale, y_axis, y_extent, y_scale, _ref;
    ...
    svg_graph = d3.select("#" + conf.id).append('svg').attr('width', CONTAINER_DIMENSIONS.width).attr('height', CONTAINER_DIMENSIONS.height).append('g').attr('transform', "translate(" + MARGINS.left + "," + MARGINS.top + ")").attr('id', 'graph_container');
    Z_QUANTITY = {
      name: 'time',
    ...
    all_graphs = d3.merge((function() {
      ...
      for (id in _ref) {
        graph = _ref[id];
        _results.push(graph.data);
      }
      return _results;
    })());
    x_extent = d3.extent(all_graphs, function(d) {
      return d[X_QUANTITY.name];
    });
    x_scale = d3.scale.linear().range([0, GRAPH_DIMENSIONS.width]).domain(x_extent);
    y_extent = d3.extent(all_graphs, function(d) {
      return d[Y_QUANTITY.name];
    });
    y_scale = d3.scale.linear().range([GRAPH_DIMENSIONS.height, 0]).domain(y_extent);
    x_axis = d3.svg.axis().scale(x_scale).tickSubdivide(3);
    y_axis = d3.svg.axis().scale(y_scale).orient('left').tickSubdivide(3);
    svg_graph.append('g').attr('class', 'x axis').attr('transform', "translate(0," + GRAPH_DIMENSIONS.height + ")").call(x_axis);
    svg_graph.append('g').attr('class', 'y axis').call(y_axis);
    d3.select('.y.axis').append('text').attr('text-anchor', 'middle').text("" + Y_QUANTITY.label + " (" + Y_QUANTITY.unit + ")").attr('transform', 'rotate(-270,0,0)').attr('x', GRAPH_DIMENSIONS.height / 2).attr('y', 50);
    d3.select('.x.axis').append('text').attr('text-anchor', 'middle').text("" + X_QUANTITY.label + " (" + X_QUANTITY.unit + ")").attr('x', GRAPH_DIMENSIONS.width / 2).attr('y', MARGINS.bottom - MARGINS.top);
    toggle_chart = function(id) {
      var chart;

      chart = d3.select("#" + id + "-chart");
      if (chart.empty()) {
        return draw_chart(conf.graphs[id], id);
      } else {
        return chart.remove();
      }
    };
    toggle_graph = function(id) {
      graph = d3.select("#" + id + "-graph");
      if (graph.empty()) {
        return draw_graph(conf.graphs[id], id);
      } else {
        return graph.remove();
      }
    };
    draw_graph = function(graph, id) {
      var g, line;
      ...
      g = d3.select('#graph_container').append('g').attr('id', "" + id + "-graph").attr('class', id);
      g.append('path').attr('d', line(graph.data)).attr('fill', 'none').attr('stroke', graph.color);
      return toggle_tangents(id);
    };
    draw_chart = function(graph, id) {
      var g;

      g = d3.select('#graph_container').append('g').attr('id', "" + id + "-chart").attr('class', id);
      g.selectAll('rect-bar').data(graph.data).enter().append('rect').attr('x', function(d) {
        return x_scale(d[X_QUANTITY.name]);
      }).attr('y', function(d) {
        return y_scale(d[Y_QUANTITY.name]);
      }).attr('width', x_scale(X_QUANTITY.step)).attr('height', function(d) {
        return GRAPH_DIMENSIONS.height - y_scale(d[Y_QUANTITY.name]);
      }).attr('fill', graph.color).attr('stroke', graph.color);
      return toggle_tangents(id);
    };
    toggle_tangents = function(id) {
      graph = d3.select("#" + id + "-tangents");
      if (graph.empty()) {
        return draw_tangents(conf.graphs[id], id);
      } else {
        return graph.remove();
      }
    };
    tangent = d3.select('#graph_container').append('line').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', 0).attr('stroke', 'black').attr('stroke-width', 1);
    draw_tangents = function(graph, id) {
      var g,
        _this = this;

      g = d3.select('#graph_container').append('g').attr('id', "" + id + "-tangents").attr('class', id);
      return g.selectAll('tangent').data(graph.data).enter().append('circle').attr('cx', function(d) {
        return x_scale(d[X_QUANTITY.name]);
      }).attr('cy', function(d) {
        return y_scale(d[Y_QUANTITY.name]);
      }).attr('r', 3).attr('fill', 'white').attr('fill-opacity', 0).attr('stroke', 'none').on('mouseover', function(d, i) {
        var LENGTH, dd, next, prev;

        prev = {
          x: i > 0 ? graph.data[i - 1][X_QUANTITY.name] : d[X_QUANTITY.name],
          y: i > 0 ? graph.data[i - 1][Y_QUANTITY.name] : d[Y_QUANTITY.name]
        };
        next = {
          x: i < graph.data.length ? graph.data[i + 1][X_QUANTITY.name] : d[X_QUANTITY.name],
          y: i < graph.data.length ? graph.data[i + 1][Y_QUANTITY.name] : d[Y_QUANTITY.name]
        };
        dd = {
          x: next.x - prev.x,
          y: next.y - prev.y
        ...
        LENGTH = 10;
        return tangent.attr('x1', x_scale(d[X_QUANTITY.name] - dd.x * LENGTH)).attr('y1', y_scale(d[Y_QUANTITY.name] - dd.y * LENGTH)).attr('x2', x_scale(d[X_QUANTITY.name] + dd.x * LENGTH)).attr('y2', y_scale(d[Y_QUANTITY.name] + dd.y * LENGTH));
      });
    };
    _ref = conf.graphs;
    for (id in _ref) {
...
(function() {
  ...
  Glass = (function() {
    ...
    Glass.prototype.height_at_volume = function(volume, floor) {
    ...
    Glass.prototype.get_data = function(flow_rate) {
    ...
    Glass.prototype.get_hdata = function(flow_rate, from_bowl) {
