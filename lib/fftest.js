...
(function() {
  ...
  FlaskFiller = (function() {
    function FlaskFiller(config) {
      ...
      this.container.attr('class', 'accordion');
      this.simulation_container = this._create_accordion_group(this.container, 'simulation', 'Glazen vullen simulatie');
      this.simulation = new Simulation(this, this.simulation_container, config);
      this.table_container = this._create_accordion_group(this.container, 'table', 'Glazen');
      this.table = new GlassTable(this, this.table_container, config);
      this.graph_container = this._create_accordion_group(this.container, 'graph', 'Grafiek');
    ...
    FlaskFiller.prototype._create_accordion_group = function(parent, name, heading_text, open) {
      var content, content_container, content_id, group, heading, parent_id;
      if (open == null) {
        open = true;
      }
      parent_id = parent.attr('id');
      content_id = "" + parent_id + "-" + name;
      group = parent.append('div').classed('accordion-group', true);
      heading = group.append('div').classed('accordion-heading', true).append('a').classed('accordion-toggle', true).attr('data-toggle', 'collapse').attr('data-parent', "#" + content_id).attr('href', "#" + content_id).text(heading_text);
      content_container = group.append('div').attr('id', content_id).classed('accordion-body', true).classed('collapse', true);
      if (open) {
        content_container.attr('class', 'accordion-body collapse in');
      }
      content = content_container.append('div').classed('accordion-inner', true);
      return content;
    };
...
(function() {
  ...
  Grapher = (function() {
    ...
    Grapher.prototype.add_graph = function(glass) {
      ...
      this.glasses.push(glass);
      this.compute_scales();
      return this.toggle_graph(glass);
    };
    ...
    Grapher.prototype.compute_scales = function() {
      ...
      d3.select('.y.axis').append('text').attr('text-anchor', 'middle').text("" + this.Y_QUANTITY.label + " (" + this.Y_QUANTITY.unit + ")").attr('transform', 'rotate(-270,0,0)').attr('x', this.GRAPH_DIMENSIONS.height / 2).attr('y', 50);
      return d3.select('.x.axis').append('text').attr('text-anchor', 'middle').text("" + this.X_QUANTITY.label + " (" + this.X_QUANTITY.unit + ")").attr('x', this.GRAPH_DIMENSIONS.width / 2).attr('y', this.MARGINS.bottom - this.MARGINS.top);
    };
    ...
    Grapher.prototype.draw_graph = function(glass, id) {
      ...
      g.append('path').attr('d', line(glass.graph.data)).attr('class', 'graph').attr('fill', 'none').attr('stroke', glass.color);
      return this.toggle_tangents(glass);
    };
    ...
    Grapher.prototype.draw_chart = function(glass, id) {
      ...
      g.selectAll('rect-bar').data(glass.graph.data).enter().append('rect').attr('x', function(d) {
        return _this.x_scale(d[_this.X_QUANTITY.name]);
      }).attr('y', function(d) {
        return _this.y_scale(d[_this.Y_QUANTITY.name]);
      }).attr('width', this.x_scale(this.X_QUANTITY.step)).attr('height', function(d) {
        return _this.GRAPH_DIMENSIONS.height - _this.y_scale(d[_this.Y_QUANTITY.name]);
      }).attr('fill', glass.color).attr('stroke', glass.color);
      return this.toggle_tangents(glass);
    };
...
(function() {
  ...
  GlassTable = (function() {
    ...
    GlassTable.prototype.add_row = function(glass) {
      ...
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-fill").classed('icon-play', true).on('click', this.filling(glass));
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-empty").classed('icon-backward', true).on('click', this.empty(glass));
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-full").classed('icon-forward', true).on('click', this.full(glass));
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-edit").classed('icon-wrench', true);
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-showgraph").classed('icon-picture', true).on('click', this.graph(glass));
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-showchart").classed('icon-signal', true).on('click', this.chart(glass));
      row.append('td').append('button').append('i').attr('id', "" + glass.name + "-remove").classed('icon-remove-sign', true).on('click', function() {
    ...
    GlassTable.prototype._create_add_menu = function(place) {
      ...
      add_button = place.append('div').classed('btn-group', true);
    ...
    GlassTable.prototype._create_foot = function() {
      ...
      foot.append('td');
      foot.append('td').append('button').append('i').classed('icon-backward', true).on('click', function() {
      ...
      foot.append('td').append('button').append('i').classed('icon-forward', true).on('click', function() {
        var glass, _i, _len, _ref, _results;
        _ref = _this.glasses;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      foot.append('td');
      foot.append('td');
      foot.append('td').append('button').append('i').classed('icon-remove-sign', true).on('click', function() {
        var glass, _i, _len, _ref, _results;
        _ref = _this.glasses;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
...
(function() {
  ...
  Simulation = (function() {
    function Simulation(flaskfiller, container, config) {
      var HEIGHT, MM_RULER, RULER_LENGTH, RULER_WIDTH, WIDTH, glass, hruler, vruler, _i, _len, _ref;
      this.flaskfiller = flaskfiller;
      this.config = config;
      this.selected = {
        selected: null
      ...
      this.canvas = Raphael('simulation', WIDTH, HEIGHT);
      console.log("sdsdf");
      this.glasses = [];
      _ref = this.config.glasses;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      vruler = new WVerticalRuler(this.canvas, 0, 0, RULER_WIDTH, RULER_LENGTH, MM_RULER);
      hruler = new WHorizontalRuler(this.canvas, RULER_WIDTH, RULER_LENGTH, RULER_LENGTH, RULER_WIDTH, MM_RULER);
    }

    Simulation.prototype.add_glass = function(glass) {
      var wglass;
      wglass = new WGlass(this.canvas, 0, 0, glass.glass, {
        fill: glass.color
      });
...
(function() {
  ...
  WGlass = (function(_super) {
    ...
    WGlass.prototype._draw = function() {
      var base, bowl, max_ml_representation, max_x, max_y, maxpoint, tappath, _ref, _ref1;
      ...
      this.tap = this.canvas.path(tappath);
      this.tap.attr({
        stroke: '#bbb',
        fill: '#ddd',
        'fill-opacity': 0.2,
...
},{"./wruler":13,"../widget":10}],9:[function(require,module,exports){
// Generated by CoffeeScript 1.6.3
/*
(c) 2012, Huub de Beer (H.T.de.Beer@gmail.com)
*/
...
},{"../widget":10,"./measure_line":11}],13:[function(require,module,exports){
// Generated by CoffeeScript 1.6.3
/*
(c) 2012, Huub de Beer (H.T.de.Beer@gmail.com)
*/
