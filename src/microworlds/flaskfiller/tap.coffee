...
Tap = class
  ...
  constructor: (@x, @y, properties) ->
    ...
    @BUTTON_WIDTH = 26
    Buttons.Button.set_width @BUTTON_WIDTH
    @BUTTON_SEP = 5
    @TIME_LABEL_SEP = 5
    @TAB_SEP = 5
    @GROUP_SEP = 15

    Buttons.Button.set_base_path @spec.icon_path
    @actions = {
      start:
        button:
          type: 'group'
  ...
  draw_buttons: ->
    ...
    for name, action of @actions
      if name in @prop.buttons
        ...
        switch button.type
          when 'action'
            @buttons.name = new Buttons.ActionButton @paper,
              x: x
              y: y
              icon: button.icon
          ...
          when 'switch'
            @buttons.name = new Buttons.SwitchButton @paper,
              x: x
              y: y
              icon: button.icon
              tooltip: button.tooltip
    ...
    for name, optiongroup of optiongroups
      buttongroup = new Buttons.ButtonGroup @paper, optiongroup
